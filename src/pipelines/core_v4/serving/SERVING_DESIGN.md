...
| D27 | **Text cleaning at export** (`hm_clean` in `export/sql/00_macros.sql`, revised 2026-09-21 after the first cluster export left HTML in the text). Entities decoded in **3 passes** (real titles are triple-escaped: `&amp;amp;#8220;`), everything except `&amp;` first and `&amp;` last; then a **whitelist** of tags is stripped: inline tags (sub sup i b em strong u small span a italic bold sc ... , MathML with or without prefix, `inline-formula`, `tex-math`, `jats:*`) -> `''` (`CO<sub>2</sub>` -> `CO2`), block tags (p br div li ul ol h1-6 tr td table title ...) -> `' '`; unknown `<...>` and comparisons (`p < 0.05`) stay; whitespace collapsed, empty -> NULL; idempotent. Applied to: works title/publisher/container_name (authors: entity decode only), projects title/summary/keywords/acronym/subjects/`fundings` text, organisation names/alternativeNames/address text, grant/stream ids and descriptions, minority names, `name_key` input, and `api/publishers.json` (built from the cleaned values; raw variants that clean to the same string merge). Not applied to urls, ids, doi, codes. Residue left on purpose: structured-abstract pseudo-tags (`<Background >`, ~600 summaries), `R&D;`-like text, malformed `&955;`. Real-data numbers: `agent_job/CLEAN_VERIFY.md`. A change here means a full export rerun (`--force` / fresh `--out`). Projects: `summary` is NULL for 86%, `acronym` NULL for 97%: UI must not promise abstract search. | default (applied) |
