...
SELECT f.fundingStream.id AS id,
       coalesce(nullif(split_part(f.fundingStream.id, '::', 1), ''), any_value(f.shortName)) AS funder,
       nullif(split_part(f.fundingStream.id, '::', 2), '') AS programme,
       nullif(split_part(f.fundingStream.id, '::', 3), '') AS action,
       any_value(f.fundingStream.description) AS description,
       any_value(f.name) AS funder_name, any_value(f.jurisdiction) AS jurisdiction,
       count(DISTINCT pid)::INTEGER AS project_count,
       count(DISTINCT pid) FILTER (is_ch)::INTEGER AS dch_project_count,
       sum(funded_amount_eur / n_fund) AS total_funded_eur      -- project amount split equally over its funding entries (EUR via ECB table, D14)
