...
SELECT
    ...
    m.funded_amount_eur,                       -- PLACEHOLDER rates (D14)
    m.funded_eur_per_org,                      -- PLACEHOLDER equal split (D13)
    ...
    list_distinct(list_transform(coalesce(p.fundings, []), lambda f: f.shortName)) AS funder_short,
